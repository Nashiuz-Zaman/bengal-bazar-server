...
export const forgotPassword = async (email: string) => {
  ...
  const user = await findUserForAuthInDb(email);
...
export const resetPassword = async (token: string, newPassword: string) => {
  ...
  const resetRecord = await findValidPasswordResetTokenInDb(token);
  ...
  await updatePasswordInDb(resetRecord.userId, passwordHash);

  // 4. Delete the used reset token
  await deletePasswordResetTokenInDb(token);
};
